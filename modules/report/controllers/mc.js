// import admin
angular.module('report').controller('mcCtrl', function ($rootScope, $http, $scope, $location, $routeParams, $route, $filter) {
        ...
        sale.forEach(function (value, key) {
              ...
              $scope.$watch("currentPage + numPerPage",
                  function () {
                      ...
                      if ($scope.filterUser >= $scope.mcList.length)
                          $scope.filterUser = $scope.mcList.length;
                      $scope.filteredTodos = $scope.mcList.slice(begin, end);
                  });

              $scope.obj_Main = $scope.mcList;
              $scope.loading = 'Done';
              // $scope.$apply(); 
      })
      .error(function(data) 
        ...
        if ($scope.searchtext !== "") {
            for (var i = 0; i < $scope.obj_Main.length; i++) {
                if (String($scope.obj_Main[i].patient_id).toLowerCase().includes($scope.searchtext.toLowerCase())
                    ...
                    $scope.mcList.push($scope.obj_Main[i]);
                }
            }
        }
        else {
            $scope.mcList = [];
            $scope.mcList = $scope.obj_Main;
        }
