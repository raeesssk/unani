// import admin
angular.module('report').controller('totalunmatchedCtrl', function ($rootScope, $http, $scope, $location, $routeParams, $route, $filter) {
  ...
  $scope.getAll = function () {
      ...
      .success(function(sale)
      {
        sale.forEach(function (value, key) {
                  $scope.totalunmatchedList.push(value);
              });
              ...
              $scope.$watch("currentPage + numPerPage",
                  function () {
                      ...
                      if ($scope.filterUser >= $scope.totalunmatchedList.length)
                          $scope.filterUser = $scope.totalunmatchedList.length;
                      $scope.filteredTodos = $scope.totalunmatchedList.slice(begin, end);
                  });

              $scope.obj_Main = $scope.totalunmatchedList;
              $scope.loading = 'Done';
              // $scope.$apply(); 
      })
      .error(function(data) 
        ...
        if ($scope.searchtext !== "") {
            for (var i = 0; i < $scope.obj_Main.length; i++) {
                if (String($scope.obj_Main[i].patient_id).toLowerCase().includes($scope.searchtext.toLowerCase())
                    ...
                    $scope.totalunmatchedList.push($scope.obj_Main[i]);
                }
            }
        }
        else {
            $scope.totalunmatchedList = [];
            $scope.totalunmatchedList = $scope.obj_Main;
        }
