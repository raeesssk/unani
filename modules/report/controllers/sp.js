// import admin
angular.module('report').controller('spCtrl', function ($rootScope, $http, $scope, $location, $routeParams, $route, $filter) {
        ...
        sale.forEach(function (value, key) {
              ...
              $scope.$watch("currentPage + numPerPage",
                  function () {
                      ...
                      if ($scope.filterUser >= $scope.spList.length)
                          $scope.filterUser = $scope.spList.length;
                      $scope.filteredTodos = $scope.spList.slice(begin, end);
                  });

              $scope.obj_Main = $scope.spList;
              $scope.loading = 'Done';
              // $scope.$apply(); 
      })
      .error(function(data) 
        ...
        if ($scope.searchtext !== "") {
            for (var i = 0; i < $scope.obj_Main.length; i++) {
                if (String($scope.obj_Main[i].patient_id).toLowerCase().includes($scope.searchtext.toLowerCase())
                    ...
                    $scope.spList.push($scope.obj_Main[i]);
                }
            }
        }
        else {
            $scope.spList = [];
            $scope.spList = $scope.obj_Main;
        }
