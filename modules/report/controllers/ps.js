// import admin
angular.module('report').controller('psCtrl', function ($rootScope, $http, $scope, $location, $routeParams, $route, $filter) {
        ...
        sale.forEach(function (value, key) {
              ...
              $scope.$watch("currentPage + numPerPage",
                  function () {
                      ...
                      if ($scope.filterUser >= $scope.psList.length)
                          $scope.filterUser = $scope.psList.length;
                      $scope.filteredTodos = $scope.psList.slice(begin, end);
                  });

              $scope.obj_Main = $scope.psList;
              $scope.loading = 'Done';
              // $scope.$apply(); 
      })
      .error(function(data) 
        ...
        if ($scope.searchtext !== "") {
            for (var i = 0; i < $scope.obj_Main.length; i++) {
                if (String($scope.obj_Main[i].patient_id).toLowerCase().includes($scope.searchtext.toLowerCase())
                    ...
                    $scope.psList.push($scope.obj_Main[i]);
                }
            }
        }
        else {
            $scope.psList = [];
            $scope.psList = $scope.obj_Main;
        }
