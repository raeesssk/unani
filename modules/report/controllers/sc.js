// import admin
angular.module('report').controller('scCtrl', function ($rootScope, $http, $scope, $location, $routeParams, $route, $filter) {
        ...
        sale.forEach(function (value, key) {
              ...
              $scope.$watch("currentPage + numPerPage",
                  function () {
                      ...
                      if ($scope.filterUser >= $scope.scList.length)
                          $scope.filterUser = $scope.scList.length;
                      $scope.filteredTodos = $scope.scList.slice(begin, end);
                  });

              $scope.obj_Main = $scope.scList;
              $scope.loading = 'Done';
              // $scope.$apply(); 
      })
      .error(function(data) 
        ...
        if ($scope.searchtext !== "") {
            for (var i = 0; i < $scope.obj_Main.length; i++) {
                if (String($scope.obj_Main[i].patient_id).toLowerCase().includes($scope.searchtext.toLowerCase())
                    ...
                    $scope.scList.push($scope.obj_Main[i]);
                }
            }
        }
        else {
            $scope.scList = [];
            $scope.scList = $scope.obj_Main;
        }
