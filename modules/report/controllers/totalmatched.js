// import admin
angular.module('report').controller('totalmatchedCtrl', function ($rootScope, $http, $scope, $location, $routeParams, $route, $filter) {
  ...
  $scope.getAll = function () {
      ...
      .success(function(sale)
      {
        sale.forEach(function (value, key) {
                  $scope.totalmatchedList.push(value);
              });
              ...
              $scope.$watch("currentPage + numPerPage",
                  function () {
                      ...
                      if ($scope.filterUser >= $scope.totalmatchedList.length)
                          $scope.filterUser = $scope.totalmatchedList.length;
                      $scope.filteredTodos = $scope.totalmatchedList.slice(begin, end);
                  });

              $scope.obj_Main = $scope.totalmatchedList;
              $scope.loading = 'Done';
              // $scope.$apply(); 
      })
      .error(function(data) 
        ...
        if ($scope.searchtext !== "") {
            for (var i = 0; i < $scope.obj_Main.length; i++) {
                if (String($scope.obj_Main[i].patient_id).toLowerCase().includes($scope.searchtext.toLowerCase())
                    ...
                    $scope.totalmatchedList.push($scope.obj_Main[i]);
                }
            }
        }
        else {
            $scope.totalmatchedList = [];
            $scope.totalmatchedList = $scope.obj_Main;
        }
