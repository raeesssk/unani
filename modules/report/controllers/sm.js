// import admin
angular.module('report').controller('smCtrl', function ($rootScope, $http, $scope, $location, $routeParams, $route, $filter) {
        ...
        sale.forEach(function (value, key) {
              ...
              $scope.$watch("currentPage + numPerPage",
                  function () {
                      ...
                      if ($scope.filterUser >= $scope.smList.length)
                          $scope.filterUser = $scope.smList.length;
                      $scope.filteredTodos = $scope.smList.slice(begin, end);
                  });

              $scope.obj_Main = $scope.smList;
              $scope.loading = 'Done';
              // $scope.$apply(); 
      })
      .error(function(data) 
        ...
        if ($scope.searchtext !== "") {
            for (var i = 0; i < $scope.obj_Main.length; i++) {
                if (String($scope.obj_Main[i].patient_id).toLowerCase().includes($scope.searchtext.toLowerCase())
                    ...
                    $scope.smList.push($scope.obj_Main[i]);
                }
            }
        }
        else {
            $scope.smList = [];
            $scope.smList = $scope.obj_Main;
        }
