// import admin
angular.module('report').controller('csCtrl', function ($rootScope, $http, $scope, $location, $routeParams, $route, $filter) {
        ...
        sale.forEach(function (value, key) {
              ...
              $scope.$watch("currentPage + numPerPage",
                  function () {
                      ...
                      if ($scope.filterUser >= $scope.csList.length)
                          $scope.filterUser = $scope.csList.length;
                      $scope.filteredTodos = $scope.csList.slice(begin, end);
                  });

              $scope.obj_Main = $scope.csList;
              $scope.loading = 'Done';
              // $scope.$apply(); 
      })
      .error(function(data) 
        ...
        if ($scope.searchtext !== "") {
            for (var i = 0; i < $scope.obj_Main.length; i++) {
                if (String($scope.obj_Main[i].patient_id).toLowerCase().includes($scope.searchtext.toLowerCase())
                    ...
                    $scope.csList.push($scope.obj_Main[i]);
                }
            }
        }
        else {
            $scope.csList = [];
            $scope.csList = $scope.obj_Main;
        }
